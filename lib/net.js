...
exports.create = function (options) {
    ...
    fs.unlink(sktfile, function (err) {
      ...
      __listen.call(that, sktfile, function () {
        ...
        that.on('connection', function (socket) {
          ...
          Parser.streamWrapper(function (buf) {
            if (buf.length < 6) {
              return;
            }

            if (String(buf).indexOf('[NAE]:') !== 0) {
              this.emit('complete', buf);
              return;
            }

            if (buf.length >= 10) {
              var len = buf.readUInt8(6);
              if (buf.length >= 10 + len) {
                try {
                  var res = JSON.parse(buf.toString('utf8', 10, 10 + len));
                  for (var i in res) {
                    socket[i] = res[i];
                  }
                } catch (e) {
                }
                this.emit('complete', buf.slice(10 + len));
              }
            }
          }, 1000).call(null, socket, function () {
            socket.end();
            debug('socket timeout after 1000 ms.');
          });
        });
        if ('function' === (typeof args[args.length - 1])) {
  ...
  Server.prototype.address = function () {
    ...
    if (this.__fakePort) {
      return {
        'port' : this.__fakePort, 
        'family' : 'IPv4',
        'address' : _options.address || '0.0.0.0'
      };
    }
